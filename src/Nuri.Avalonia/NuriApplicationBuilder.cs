using System;
using Avalonia.Controls;
using Avalonia.Input;
using Avalonia.Threading;
using Nuri.Runtime.Diagnostics;
using Nuri.UI.Dsl;

namespace Nuri.Avalonia
{
    public sealed class NuriApplicationBuilder<TComponent> : INuriDebugHost
        where TComponent : Component, new()
    {
        private readonly object _syncRoot = new object();
        private readonly string[] _args;
        private readonly string _title;
        private readonly double _width;
        private readonly double _height;
        private Window? _window;
        private DebugKey _debugKey = DebugKey.F12;
        private Action? _openInspector;
        private bool _started;
        private bool _closed;

        internal NuriApplicationBuilder(string[] args, string title, double width, double height)
        {
            _args = args ?? throw new ArgumentNullException(nameof(args));
            _title = title ?? throw new ArgumentNullException(nameof(title));
            _width = width;
            _height = height;
        }

        public bool HasStarted
        {
            get
            {
                lock (_syncRoot)
                    return _started;
            }
        }

        public bool IsClosed
        {
            get
            {
                lock (_syncRoot)
                    return _closed;
            }
        }

        public void Run()
        {
            lock (_syncRoot)
            {
                if (_started)
                    throw new InvalidOperationException("This Nuri application builder has already started.");
                if (_closed)
                    throw new ObjectDisposedException(GetType().FullName);

                _started = true;
            }

            try
            {
                NuriApplication.Run(
                    _args,
                    NuriApplication.CreateRoot<TComponent>(_title, _width, _height),
                    OnWindowCreated);
            }
            catch
            {
                lock (_syncRoot)
                    _closed = true;
                throw;
            }
        }

        public void SetDebugShortcut(DebugKey key, Action openInspector)
        {
            if (key < DebugKey.F1 || key > DebugKey.F12)
                throw new ArgumentOutOfRangeException(nameof(key), key, "DebugKey must be between F1 and F12.");
            if (openInspector == null)
                throw new ArgumentNullException(nameof(openInspector));

            lock (_syncRoot)
            {
                if (_closed)
                    throw new ObjectDisposedException(GetType().FullName);

                _debugKey = key;
                _openInspector = openInspector;
            }
        }

        public RuntimeSnapshot CaptureSnapshot()
        {
            Window? window;
            bool closed;
            lock (_syncRoot)
            {
                window = _window;
                closed = _closed;
            }

            if (window == null || closed || Dispatcher.UIThread.CheckAccess())
                return NuriDiagnostics.GetSnapshot();

            return Dispatcher.UIThread.InvokeAsync(NuriDiagnostics.GetSnapshot).GetAwaiter().GetResult();
        }

        public void HighlightComponent(string? componentId)
        {
            // Avalonia does not yet retain a component-to-control index for highlighting.
        }

        private void OnWindowCreated(Window window)
        {
            lock (_syncRoot)
                _window = window;

            window.KeyDown += OnKeyDown;
            window.Closed += OnWindowClosed;
        }

        private void OnKeyDown(object? sender, KeyEventArgs args)
        {
            if (args.KeyModifiers != KeyModifiers.None)
                return;

            DebugKey debugKey;
            Action? openInspector;
            lock (_syncRoot)
            {
                if (_closed)
                    return;

                debugKey = _debugKey;
                openInspector = _openInspector;
            }

            if (openInspector == null || args.Key != ToAvaloniaKey(debugKey))
                return;

            args.Handled = true;
            openInspector();
        }

        private void OnWindowClosed(object? sender, EventArgs args)
        {
            if (sender is Window window)
                window.KeyDown -= OnKeyDown;

            lock (_syncRoot)
            {
                _closed = true;
                _openInspector = null;
            }
        }

        private static Key ToAvaloniaKey(DebugKey key)
        {
            return key switch
            {
                DebugKey.F1 => Key.F1,
                DebugKey.F2 => Key.F2,
                DebugKey.F3 => Key.F3,
                DebugKey.F4 => Key.F4,
                DebugKey.F5 => Key.F5,
                DebugKey.F6 => Key.F6,
                DebugKey.F7 => Key.F7,
                DebugKey.F8 => Key.F8,
                DebugKey.F9 => Key.F9,
                DebugKey.F10 => Key.F10,
                DebugKey.F11 => Key.F11,
                DebugKey.F12 => Key.F12,
                _ => throw new ArgumentOutOfRangeException(nameof(key), key, "DebugKey must be between F1 and F12.")
            };
        }
    }
}
