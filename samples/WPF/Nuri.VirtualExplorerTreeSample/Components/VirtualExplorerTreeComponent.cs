...
namespace Nuri.VirtualExplorerTreeSample.Components;

public sealed class VirtualExplorerTreeComponent : Component
{
    ...
    public override IElement Render()
    {
        var (state, setState) = useState(new ExplorerState(
            ...
            "file-0-0",
            "10,101 visible nodes; native rows are created only for the viewport."));
        ...
        return
            Grid(
                Header(visibleRows.Count)
                    .Row(0),
                Grid(
                    TreePanel(VirtualizedItems(visibleRows, row => row.Node.Id, RowExtent, row => NodeRow(row, state.SelectedId, Select, Toggle)))
                        .Column(0),
    ...
    {
        return Div(
                Text("Virtual Explorer Tree")
                    .FontSize(28)
                    .FontWeight(FontWeightValue.Bold)
                    .FontColor("#111827"),
                Text($"{TotalNodeCount:N0} generated nodes, {visibleCount:N0} currently visible, fixed {RowExtent:0}px item extent.")
                    .FontSize(13)
                    .FontColor("#6b7280")
        ...
        Action collapseAll)
    {
        return Div(
                ...
                Text($"{visibleCount:N0} virtual rows")
                    ...
                    .FontColor("#1d4ed8")
                    .Margin(top: 14),
                Text("Only rows intersecting the viewport are materialized as WPF controls. Scroll quickly to observe recycling.")
                    .FontColor("#4b5563")
