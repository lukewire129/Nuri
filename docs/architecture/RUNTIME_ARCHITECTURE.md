...
Duxel development must preserve Core neutrality. The physical and solution folder `Nuri.Duxel` contains four projects: `src/Nuri.Duxel/Nuri.Duxel` owns immediate-mode frame projection and Duxel-specific property, event, and animation materialization, `src/Nuri.Duxel/Nuri.Duxel.Windows` owns Windows application and frame-loop integration, `src/Nuri.Duxel/Nuri.Duxel.Diagnostics` owns the Duxel diagnostics package, and `src/Nuri.Duxel/Nuri.Duxel.PreviewHost` owns the out-of-process Visual Studio/VS Code preview host. None of these projects may reshape Core around retained native-control assumptions. The preview extensions select WPF or Duxel from the selected project's transitive references; a project that references both renderers is rejected as ambiguous. The Visual Studio VSIX and VS Code prepublish output both package the WPF and Duxel hosts.
...
Duxel preview source-save reloads use a 150 ms debounce. `PreviewBuildService`'s Roslyn path must honor the project's `OutputType` and include dependency assemblies from the restored project output so `WinExe` and top-level-statement Duxel projects do not fall back to `dotnet build` on every edit. On the 2026-07-21 workstation, repeated `Nuri.DuxelSample` partial reloads reduced command-to-`Previewing` median latency from 3,161.9 ms (5 runs) to 596.0 ms (7 runs); an actual one-line `Text` string change completed in 359.0 ms.
...
The intended logical component identity includes parent, component type, and key. The current runtime node ID is derived from parent plus key or position, so replacing a component CLR type at the same location is not independently detected when both components render compatible virtual roots. Treat this as a known runtime gap rather than relying on state preservation across component-type replacement.
...
`Nuri.Duxel.Windows` selects `DuxelPerformanceProfile.Render` and `DuxelTextRenderingMode.DirectText` for hosted applications. With Duxel's default `MsaaSamples = 0`, `Render` resolves to 1x MSAA instead of the `Display` profile's 4x MSAA, prioritizing UI throughput over multisample edge quality without changing VSync. `DirectText` uses the Windows platform text backend for every glyph and is the quality-first default; the faster `Auto` atlas path is not selected because its glyph appearance can differ or render incorrectly on real application text. In a 12-process 1,000-text cold measurement, `Auto` reduced first-frame projection from 15.59 ms to 11.42 ms, but launch-to-first-present changed only from 637.55 ms to 634.86 ms, so the small end-to-end gain does not justify the observed text-quality regression.

Published Duxel applications should enable `<PublishReadyToRun>true</PublishReadyToRun>` to remove most first-frame JIT work; source builds remain managed and do not gain this benefit. Projects under `samples/Duxel` and `perf/Nuri.DuxelWindowsPerformance` enable ReadyToRun for publish, and the samples enable `TieredCompilationQuickJitForLoops` for managed runs. In a local 1,000-text process-cold comparison, the managed first-frame Nuri CPU median was 43.13 ms across 5 processes versus 22.14 ms across 10 ReadyToRun processes; launch-to-first-present medians were 695.93 ms and 594.60 ms. ReadyToRun increased the measured managed application files from 1.63 MB to 4.10 MB, so package consumers must opt in from their executable project rather than `Nuri.Duxel.Windows` forcing the deployment tradeoff.

`VirtualEntry` shares immutable empty property, event, and animation maps and does not allocate child storage for leaf entries. The Duxel adapter also omits temporary lists for empty element collections. In the 1,000-text headless first-frame benchmark, mean allocation fell from 2,265.37 KB to 1,988.42 KB, a 12.2% reduction, without changing patch counts.
...
`NuriApplication` suppresses the Duxel window after its creation callback and releases it after the initial Nuri projection, commit, and effect flush. This keeps the window hidden during Vulkan device, swapchain, pipeline, and font startup, so the normal user-visible transition is directly from no window to the first populated frame instead of a long empty client area. A five-second native timer releases the window if the renderer never reaches the readiness callback. Direct `DuxelWindowsApp` hosts are unchanged. On the local performance host, the upstream Duxel create/show sequence was visible to `IsWindowVisible` for 9-13 ms before the callback hid it, then remained hidden until the first-frame boundary; removing that final pre-callback interval requires a Duxel `StartHidden` window option.

`NuriApplication.ShowModeless(Func<Action, IElement>, ...)` creates a separate native Duxel window through `DuxelWindowsApp.ShowModeless`; it does not call the WPF `Attach` path. Each modeless window owns an independent `DuxelAppSession`, `NuriDuxelScreen`, input bridge, runtime tree, and STA render thread. The supplied close action exits only that window's session, the optional owner HWND establishes native ownership, and window shutdown disposes the screen and unregisters its hot-reload, diagnostics, hook, and input state without stopping the main window.

The Windows host records ordered pointer, wheel, key, text, focus, and actual `WM_SIZE` resize events in `DuxelInputEventQueue`; consecutive pointer-move and resize samples may coalesce, but semantic transitions and wheel samples retain their order and event-time position. Nuri does not consume non-client resize hit tests, call `SetWindowPos`, create overlay windows, estimate completion with timers, or wait for a presentation callback. Duxel owns swapchain recreation, command submission, and present. For the published Duxel `0.2.12-preview` contract, the bridge observes `WM_SIZING` without changing its proposed rectangle and forwards the message to Duxel. Duxel publishes the proposed client size, renders and presents the matching frame synchronously, then lets Windows apply the bounds. The subsequent actual `WM_SIZE` records the final neutral resize event. The preview host may also mirror proposed client bounds for its explicit `contentScaleProvider`. This handshake alone does not guarantee pixel-stable content and must not be described as a zoom fix without a real slow-drag measurement.

Normal Windows application roots arrange against the current Duxel frame's `GetMainViewport().WorkPos` and `WorkSize`. Nuri emits geometry and clips in that frame's logical coordinate space and does not request swapchain recreation. The published Duxel `0.2.12-preview` Vulkan projection divides vertex coordinates by the proposed display size while the swapchain keeps its previous extent during interactive resize. In a real 12-step slow drag from a `744x665` to a `924x725` outer window, an explicit `120x34` Nuri button progressively measured `117x34` through `96x31`, then returned to `120x34` only after resize exit. This is a user-visible shrink and is not acceptable normal behavior. The headless Nuri renderer test proves only that logical fixed-child geometry remains `120x34`; it cannot validate the Vulkan render-target projection.

The published `Duxel.App` and `Duxel.Windows.App` `0.2.12-preview` packages are the only Duxel dependencies used by Nuri. A sibling `..\Duxel` checkout is never referenced, and there is no local-source build switch. The package public API does not expose the actual command-buffer render-target extent or a frame-bound target mapping, so Nuri cannot implement the proposed stage-two mapping accurately from `0.2.12-preview` alone. Nuri therefore continues to submit logical geometry and clips through the package API, does not infer swapchain state from HWND sizes, and does not request swapchain recreation. The measured interactive shrink remains a package-level limitation until Duxel publishes an API or behavior change that resolves it. Duxel `0.2.12-preview` separately presents valid clear-only frames, so geometry-free resize sequences can complete without changing Nuri's integration contract.

The current Nuri path submits Duxel primitives and logical clip rectangles directly; it does not use a Nuri-owned offscreen texture or custom Vulkan command buffer. Pixel accuracy is required during drag for one-pixel panel/button borders and clip edges, while text keeps Duxel's glyph-origin and atlas/direct-text policy instead of rounding every vertex. Final resize-exit accuracy remains mandatory for all geometry and text.
...
Row containers give their children one shared vertical origin and advance by the measured Row height. Duxel's implicit post-item spacing is not counted as Row content height; only the Nuri `Spacing` value separates Row children.

Duxel linear layouts default to zero item spacing, matching WPF; Duxel theme `ItemSpacing` does not become an implicit Nuri layout gap. A constrained `Block` overlays its children in the assigned bounds and propagates those bounds, matching the WPF `Grid` materialization and allowing routed content to fill a Star track. Renderer-owned Scroll uses its full assigned bounds as the content viewport, keeps scrollbar inset visual-only, and treats overflow of at most 0.5 logical pixels as layout noise rather than exposing a scrollbar.

Duxel retains intrinsic desired-size measurements by `VirtualEntry` reference across projection-only frames. A runtime tree, theme, or content-scale change invalidates those measurements; a viewport-only resize does not. Grid, Row, and Column arrangement still runs against the current `WorkSize` on every frame, so Star/Grow/Stretch slots and child positions react to the window while explicit leaf dimensions, text size, padding, and borders remain fixed logical units.

In a constrained Row or Column, `.Grow(weight)` divides the main-axis space remaining after fixed children and explicit `Spacing` in proportion to each positive weight. Duxel computes the same slots as the retained WPF and Avalonia distributed panels; an empty Grow child still consumes its slot, so spacer elements can anchor following content to the trailing edge.

Core `Text` exposes `.TextOverflow(TextOverflowValue.Clip)`, `.TextOverflow(TextOverflowValue.Ellipsis)`, and `.TextOverflow(TextOverflowValue.Wrap)` as renderer-neutral behavior; omitting the property is equivalent to `Clip`. Duxel clips text to its assigned width and height by default, applies grapheme-safe single-line ellipsis when selected, and wraps at whitespace with grapheme fallback for an overlong token. Wrapped text expands an Auto Grid row, while fixed-height bounds still clip vertical overflow. WPF and Avalonia map the same values to their native `TextWrapping` and `TextTrimming` properties.

Core `Absolute(children)` creates a `DivTypes.Absolute` layout whose children use `.Position(x, y)`, `.PositionX(...)`, and `.PositionY(...)` in the layout coordinate space. Position is layout placement rather than a render transform; `.Translate(...)` remains the visual transform API. Absolute surfaces should receive explicit bounds because their positioned children do not define a natural retained-panel extent. WPF and Avalonia materialize the contract with `Canvas` and retain native children when position properties patch. Duxel does not yet materialize `DivTypes.Absolute` or `PositionX`/`PositionY`; `Nuri.WorkflowSample` is therefore currently WPF-specific.

Core `Viewport(content)` is a single-content controlled 2D camera. `.ViewportOffset(x, y)` identifies the content coordinate projected at the viewport origin, and `.ViewportZoom(zoom)` applies a finite scale greater than zero, so a content point projects as `(content - offset) * zoom`. The viewport clips at its fixed layout bounds while camera property patches retain the content tree and renderer-owned camera host. WPF materializes this contract and restores offset `0,0` and zoom `1` when those properties are removed. Pointer callbacks on descendants continue to report immediate-parent content coordinates, so dragging an `Absolute` child does not apply a second zoom correction. `Nuri.WorkflowSample` uses this contract for pointer-anchored `Ctrl+Wheel` zoom and captured secondary-button pan. Avalonia and Duxel materialization remain future parity work.

Core pointer callbacks `.OnPointerDown(...)`, `.OnPointerMove(...)`, `.OnPointerUp(...)`, and `.OnPointerWheel(...)` report `X` and `Y` in the immediate parent layout coordinate space while `LocalX` and `LocalY` identify the position inside the event source. `PointerEvent` also exposes `PointerButtons`, `ChangedButton`, neutral `KeyModifiers`, convenience primary/secondary/middle pressed properties, and mutable `Handled`; `PointerWheelEvent` adds `DeltaX` and `DeltaY`. Primary down/up remains the compatibility default, while `PointerButton.Secondary` opts into secondary-button routing. `EventRouting.Bubble` remains the default and `EventRouting.Tunnel` selects the preview path. Pointer capture is separately explicit through `capturePointer: true` and `releasePointerCapture: true`. WPF materializes primary/secondary buttons, move, wheel, modifiers, capture, and handled propagation. Avalonia retains its primary-button routing baseline; secondary-button, wheel, modifier, local-coordinate, and handled parity plus all Duxel pointer materialization remain future work.

The payload-free compatibility callbacks `.OnMouseDown(...)` and `.OnMouseUp(...)` accept the same optional `EventRouting` choice and also default to Bubble. They normalize to the same internal `PointerDown` and `PointerUp` event kinds while retaining the caller's original `Action`, so stable handlers do not create compatibility-wrapper event patches. Use them for simple press/release notification. Use the `PointerEvent` callbacks when code needs coordinates, primary-button state, pointer capture, or move tracking. Semantic `.OnClick(...)` remains renderer-native activation and does not expose a Tunnel option because keyboard activation must continue to produce the same click behavior.

Direct/non-Windows hosts and normal Windows application roots use `GetMainViewport().WorkSize` as the root size. This keeps Nuri projection on the exact logical work area in Duxel's current frame snapshot, including native or Duxel title-bar exclusion, and prevents a separately sampled client size from making controls appear zoomed against the resized background. Only hosts with an explicit `contentScaleProvider`, currently the Duxel preview host, override it with logical client dimensions measured initially from `GetClientRect`, predictively from `WM_SIZING`, and finally from actual `WM_SIZE` messages.
