...
- Disposing a Duxel root cancels queued frame work, clears invalidations and pending rebuild, replacement, and theme requests, disposes renderer diagnostics, disposes the runtime subtree, and unregisters the root. A modeless Duxel window additionally detaches its hot-reload, theme, input, and visibility resources without stopping other windows.
...
- `DisposeHookState` removes effect, memo, pending effect, state, and Store hook entries for the component subtree. It also disposes Store subscriptions and removes diagnostics and runtime-ancestry records.
