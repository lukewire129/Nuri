...
- The Duxel physical and solution folder is `Nuri.Duxel`. `src/Nuri.Duxel/Nuri.Duxel` targets `net8.0` and `net9.0` and references `Duxel.App` `0.2.12-preview`. `src/Nuri.Duxel/Nuri.Duxel.Windows` targets `net8.0-windows` and `net9.0-windows`, references the renderer plus `Duxel.Windows.App` `0.2.12-preview`, and owns `NuriApplication`/`DuxelAppSession` execution. These published packages are the only Duxel dependencies; sibling source checkouts are not referenced.
...
- `NuriDuxelScreen` projects root content directly into the Duxel viewport work area, so Nuri content is not represented by a draggable nested Duxel window. Direct hosts and normal Windows application roots use the current frame's `WorkPos`/`WorkSize`; do not override normal application size from a separately sampled HWND client rectangle, because two resize snapshots can make controls appear zoomed against the background. The preview host's explicit `contentScaleProvider` retains a logical client-size override derived from `GetClientRect`, proposed `WM_SIZING` bounds, and actual `WM_SIZE` messages. The native Windows title bar is already excluded; do not subtract a title-bar inset again. The Windows host must run the same `DuxelAppSession` whose `RequestFrame` callback is supplied to the screen, otherwise idle-frame skipping can delay state and animation updates.
...
- Duxel owns swapchain recreation, command submission, and present. `WindowsInputEventBridge` forwards proposed `WM_SIZING` bounds without calling `SetWindowPos` or adding a timer. A real 12-step slow drag proved that the published `0.2.12-preview` Vulkan projection visibly shrinks an explicit `120x34` Nuri button to `96x31` before resize exit; the headless logical-layout test does not cover this target mapping. The package does not expose the actual command-buffer render-target extent, so Nuri cannot add an accurate frame mapping without a new published Duxel contract. Do not report the package path as fixed.
...
- Keep `Nuri.Duxel` on `Duxel.App` and `Nuri.Duxel.Windows` on `Duxel.Windows.App`; both are pinned to `0.2.12-preview` in this slice. Do not collapse the Windows host back into the renderer package.
