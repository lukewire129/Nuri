...
- Runtime diagnostics track component render counts, duplicate keys, root patch batches grouped by `PatchOperationType`, and virtualized item/realized-row counts. WPF also records deduplicated `UnsupportedProperty` entries after every supported property fallback fails; unsupported event diagnostics remain a candidate.
...
5. Add diagnostics where they solve observed debugging problems.
   ...
   - Unsupported event warnings are the main remaining candidate; WPF unsupported property warnings are implemented.
