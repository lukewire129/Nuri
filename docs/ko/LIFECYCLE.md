...
Nuri lifecycle은 virtual tree render, diff, patch, commit, effect flush 순서로 진행됩니다. Identity, key 및 필수 regression-test 계약은 [RUNTIME_IDENTITY.md](RUNTIME_IDENTITY.md)를 참고하며 lifecycle 변경은 해당 계약을 유지해야 합니다.
...
- `NuriDuxelScreen`은 초기 projection, runtime commit 및 effect flush가 끝난 뒤 optional `initialFrameCommitted` callback을 한 번 호출합니다. Windows host는 Duxel이 frame을 submit하고 present하기 전에 이 renderer readiness signal을 사용하며, 이는 component lifecycle callback이 아닙니다.
...
- Dirty component의 기존 virtual subtree를 찾을 수 없으면 전체 rebuild로 fallback합니다.
- Patch 이후 commit된 subtree 교체에 실패해도 전체 rebuild로 fallback합니다.
...
- dependency가 있는 effect는 mount와 `Equals` 기준 dependency 변경 시 실행됩니다.
- 변경된 effect가 다시 실행되기 전에 이전 cleanup을 실행합니다.
- 초기 mount, 전체 rebuild 및 성공한 subtree rebuild 이후 `FlushPendingEffects`를 호출합니다.
