...
- `AnimatedRouter(navigation, duration, easing, routes)`는 표시 중인 route의 root opacity가 `1`에서 `0`으로 전환되는 동안 해당 route를 mount 상태로 유지합니다.
- `duration` 이후 opacity `0`에서 keyed route subtree를 교체한 다음 replacement를 `0`에서 `1`로 전환합니다.
- Exit과 entry 단계는 각각 `duration`을 사용합니다. 따라서 route cleanup은 exit 단계 이후 keyed subtree를 교체할 때 실행됩니다.
- 더 새로운 navigation 요청은 대기 중인 교체를 취소하고 최신 route를 대상으로 합니다. 현재 표시 중인 route로 돌아오면 opacity `1`을 대상으로 exit을 되돌립니다.
- 이 순서는 platform-neutral합니다. 각 renderer는 neutral opacity transition materialization과 animation frame scheduling을 계속 담당합니다.
