...
의도한 논리 component identity에는 parent, component type 및 key가 포함됩니다. 현재 runtime node ID는 parent와 key 또는 position에서 파생되므로 같은 위치에서 component CLR type을 교체해도 두 component가 호환되는 virtual root를 render하면 이를 독립적으로 감지하지 못합니다. Component-type replacement에서 state가 보존된다고 의존하지 말고 현재 runtime gap으로 취급합니다.
...
`NuriApplication.ShowModeless(Func<Action, IElement>, ...)`는 WPF `Attach` 경로를 호출하지 않고 `DuxelWindowsApp.ShowModeless`를 통해 별도의 native Duxel window를 생성합니다. 각 modeless window는 독립적인 `DuxelAppSession`, `NuriDuxelScreen`, input bridge, runtime tree 및 STA render thread를 소유합니다. 전달된 close action은 해당 window의 session만 종료하고, 선택적 owner HWND는 native ownership을 설정하며, window 종료 시 main window를 중지하지 않고 screen을 dispose하고 hot-reload, diagnostics, hook 및 input state를 등록 해제합니다.

Windows host는 순서가 있는 pointer, wheel, key, text, focus 및 실제 `WM_SIZE` resize event를 `DuxelInputEventQueue`에 기록합니다. 연속 pointer-move와 resize sample은 합칠 수 있지만 semantic transition과 wheel sample은 순서와 event 발생 시점의 위치를 유지합니다. Nuri는 non-client resize hit test를 소비하거나 `SetWindowPos`를 호출하거나 overlay window를 만들거나 timer로 완료 시점을 추정하거나 presentation callback을 기다리지 않습니다. Swapchain 재생성, command submission 및 present는 Duxel이 소유합니다. 공개 Duxel `0.2.12-preview` 계약을 위해 bridge는 proposed rectangle을 바꾸지 않은 채 `WM_SIZING`을 관찰하고 그 message를 Duxel에 전달합니다. Duxel은 proposed client size를 게시하고 일치하는 frame을 동기식으로 render/present한 다음 Windows가 bounds를 적용하게 합니다. 이후 실제 `WM_SIZE`는 최종 neutral resize event를 기록합니다. Preview host는 명시적 `contentScaleProvider`를 위해 proposed client bounds도 반영할 수 있습니다. 이 handshake만으로 pixel-stable content가 보장되지는 않으며 실제 slow-drag 측정 없이 zoom fix라고 설명해서는 안 됩니다.

일반 Windows application root는 현재 Duxel frame의 `GetMainViewport().WorkPos`와 `WorkSize`를 기준으로 arrange합니다. Nuri는 해당 frame의 logical coordinate space에서 geometry와 clip을 생성하고 swapchain 재생성을 요청하지 않습니다. 공개 Duxel `0.2.12-preview` Vulkan projection은 interactive resize 중 swapchain이 이전 extent를 유지하는 동안 vertex coordinate를 proposed display size로 나눕니다. 실제 outer window를 `744x665`에서 `924x725`로 12단계 천천히 drag한 결과 명시적 `120x34` Nuri button은 `117x34`에서 `96x31`까지 점진적으로 줄었고 resize exit 후에만 `120x34`로 돌아왔습니다. 이는 사용자가 볼 수 있는 shrink이며 정상 동작으로 허용할 수 없습니다. Headless Nuri renderer test는 logical fixed-child geometry가 `120x34`로 유지된다는 것만 증명하며 Vulkan render-target projection은 검증할 수 없습니다.

공개 `Duxel.App` 및 `Duxel.Windows.App` `0.2.12-preview` package만 Nuri의 Duxel dependency로 사용합니다. Sibling `..\Duxel` checkout은 참조하지 않으며 local-source build switch도 없습니다. Package public API는 실제 command-buffer render-target extent나 frame-bound target mapping을 노출하지 않으므로 Nuri는 `0.2.12-preview`만으로 제안된 2단계 mapping을 정확하게 구현할 수 없습니다. 따라서 Nuri는 package API를 통해 logical geometry와 clip을 계속 제출하고, HWND size로 swapchain state를 추정하지 않으며, swapchain 재생성을 요청하지 않습니다. 측정된 interactive shrink는 Duxel이 이를 해결하는 API 또는 동작 변경을 package로 공개할 때까지 package 수준의 제약으로 남습니다. Duxel `0.2.12-preview`는 별도로 유효한 clear-only frame을 present하므로 geometry가 없는 resize sequence도 Nuri 통합 계약을 바꾸지 않고 완료할 수 있습니다.

현재 Nuri 경로는 Duxel primitive와 logical clip rectangle을 직접 제출하며 Nuri 소유 offscreen texture나 custom Vulkan command buffer를 사용하지 않습니다. Drag 중 pixel accuracy는 1px panel/button border와 clip edge에 필요하며, text는 모든 vertex를 반올림하지 않고 Duxel의 glyph-origin 및 atlas/direct-text 정책을 유지합니다. Resize exit 후 최종 정확도는 모든 geometry와 text에 계속 필수입니다.
...
Duxel linear layout은 WPF와 동일하게 item spacing 기본값으로 0을 사용하며, Duxel theme `ItemSpacing`을 암시적인 Nuri layout gap으로 사용하지 않습니다. 제약된 `Block`은 배정된 bounds 안에서 child를 겹쳐 배치하고 해당 bounds를 전달하여 WPF `Grid` materialization과 일치시키며, route content가 Star track을 채울 수 있게 합니다. Renderer-owned Scroll은 배정된 전체 bounds를 content viewport로 사용하고 scrollbar inset은 시각적 요소에만 적용하며, 0.5 logical pixel 이하의 overflow는 scrollbar를 노출하지 않고 layout noise로 처리합니다.

Duxel은 projection-only frame 사이에 `VirtualEntry` reference별 intrinsic desired-size 측정값을 유지합니다. Runtime tree, theme 또는 content-scale이 바뀌면 이 측정값을 invalidate하지만 viewport만 resize될 때는 유지합니다. Grid, Row 및 Column arrange는 매 frame마다 현재 `WorkSize`를 기준으로 계속 실행하므로 Star/Grow/Stretch slot과 child 위치는 window에 반응하고 명시적인 leaf dimension, text size, padding 및 border는 고정 logical unit로 유지됩니다.

제약된 Row 또는 Column에서 `.Grow(weight)`는 fixed child와 명시적 `Spacing`을 배치하고 남은 main-axis 공간을 각각의 양수 weight 비율로 나눕니다. Duxel은 retained WPF 및 Avalonia distributed panel과 동일한 slot을 계산하며, 비어 있는 Grow child도 자신의 slot을 소비하므로 spacer element가 뒤따르는 content를 trailing edge에 고정할 수 있습니다.

Core `Text`는 renderer-neutral 동작으로 `.TextOverflow(TextOverflowValue.Clip)`, `.TextOverflow(TextOverflowValue.Ellipsis)`, `.TextOverflow(TextOverflowValue.Wrap)`을 노출하며, 이 property를 생략하면 `Clip`과 같습니다. Duxel은 기본적으로 배정된 width와 height에 맞춰 text를 clip하고, 선택 시 grapheme-safe single-line ellipsis를 적용하며, whitespace에서 wrap하되 지나치게 긴 token은 grapheme 단위로 나눕니다. Wrap된 text는 Auto Grid row를 확장하지만 fixed-height bounds에서는 vertical overflow를 계속 clip합니다. WPF와 Avalonia는 같은 값을 native `TextWrapping` 및 `TextTrimming` property로 mapping합니다.

Core `Absolute(children)`는 child가 layout coordinate space에서 `.Position(x, y)`, `.PositionX(...)` 및 `.PositionY(...)`를 사용하는 `DivTypes.Absolute` layout을 만듭니다. Position은 render transform이 아니라 layout placement이며 `.Translate(...)`는 계속 visual transform API입니다. 배치된 child가 retained panel의 natural extent를 정의하지 않으므로 Absolute surface에는 명시적인 bounds를 지정해야 합니다. WPF와 Avalonia는 이 계약을 `Canvas`로 materialize하며 position property를 patch할 때 native child를 유지합니다. Duxel은 아직 `DivTypes.Absolute` 또는 `PositionX`/`PositionY`를 materialize하지 않으므로 `Nuri.WorkflowSample`은 현재 WPF 전용입니다.

Core `Viewport(content)`는 단일 content를 가진 controlled 2D camera입니다. `.ViewportOffset(x, y)`는 viewport origin에 투영되는 content coordinate를 나타내고 `.ViewportZoom(zoom)`은 0보다 큰 유한한 scale을 적용하므로 content point는 `(content - offset) * zoom`으로 투영됩니다. Viewport는 고정된 layout bounds에서 clip하며 camera property patch는 content tree와 renderer 소유 camera host를 유지합니다. WPF는 이 계약을 materialize하고 해당 property가 제거되면 offset `0,0`과 zoom `1`을 복원합니다. Descendant의 pointer callback은 계속 immediate-parent content coordinate를 보고하므로 `Absolute` child를 drag할 때 zoom을 두 번 보정하지 않습니다. `Nuri.WorkflowSample`은 이 계약으로 pointer 중심 `Ctrl+Wheel` zoom과 capture된 secondary-button pan을 구현합니다. Avalonia와 Duxel materialization은 향후 parity 작업으로 남아 있습니다.

Core pointer callback `.OnPointerDown(...)`, `.OnPointerMove(...)`, `.OnPointerUp(...)` 및 `.OnPointerWheel(...)`은 `X`와 `Y`를 immediate parent layout coordinate space에서 보고하고 `LocalX`와 `LocalY`는 event source 내부 위치를 나타냅니다. `PointerEvent`는 `PointerButtons`, `ChangedButton`, neutral `KeyModifiers`, primary/secondary/middle pressed convenience property 및 변경 가능한 `Handled`도 노출하고 `PointerWheelEvent`는 `DeltaX`와 `DeltaY`를 추가합니다. Primary down/up은 compatibility 기본값으로 유지하며 `PointerButton.Secondary`로 secondary-button routing을 선택합니다. `EventRouting.Bubble`은 계속 기본값이고 `EventRouting.Tunnel`은 preview path를 선택합니다. Pointer capture는 `capturePointer: true`와 `releasePointerCapture: true`로 별도 명시합니다. WPF는 primary/secondary button, move, wheel, modifier, capture 및 handled propagation을 materialize합니다. Avalonia는 primary-button routing baseline을 유지하며 secondary-button, wheel, modifier, local-coordinate, handled parity와 모든 Duxel pointer materialization은 향후 작업으로 남아 있습니다.

Payload가 없는 compatibility callback `.OnMouseDown(...)`과 `.OnMouseUp(...)`도 같은 선택적 `EventRouting`을 받으며 기본값은 Bubble입니다. 이 callback들은 호출자의 원래 `Action`을 유지하면서 동일한 내부 `PointerDown` 및 `PointerUp` event kind로 normalize하므로 stable handler가 compatibility-wrapper event patch를 만들지 않습니다. 단순 press/release 알림에는 이 callback을 사용합니다. 좌표, primary-button 상태, pointer capture 또는 move tracking이 필요하면 `PointerEvent` callback을 사용합니다. Semantic `.OnClick(...)`은 renderer-native activation으로 유지하며 keyboard activation이 동일한 click 동작을 계속 생성해야 하므로 Tunnel 옵션을 노출하지 않습니다.

Direct/non-Windows host와 일반 Windows application root는 `GetMainViewport().WorkSize`를 root size로 사용합니다. 이렇게 Nuri projection이 native 또는 Duxel title bar 제외를 포함한 Duxel 현재 frame snapshot의 정확한 logical work area를 사용하게 하며, 별도로 sample한 client size 때문에 resize된 background에 비해 control이 zoom된 것처럼 보이지 않게 합니다. 명시적 `contentScaleProvider`가 있는 host, 현재는 Duxel preview host만 처음에는 `GetClientRect`, 예측 단계에서는 `WM_SIZING`, 마지막에는 실제 `WM_SIZE` message에서 측정한 logical client dimension으로 이를 override합니다.
