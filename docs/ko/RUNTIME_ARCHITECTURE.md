...
Core pointer callback `.OnPointerDown(...)`, `.OnPointerMove(...)` 및 `.OnPointerUp(...)`은 immediate parent layout coordinate space의 `X`, `Y`와 primary-button 상태를 나타내는 `IsPrimaryButtonPressed`를 가진 `PointerEvent`를 전달합니다. 기본값은 `EventRouting.Bubble`이며 ancestor 또는 control class가 callback보다 먼저 interaction을 소비하면 안 되는 경우 호출자가 `EventRouting.Tunnel`을 선택할 수 있습니다. Pointer capture는 별도로 명시합니다. Drag interaction에는 `.OnPointerDown(...)`에 `capturePointer: true`를 전달하고 `.OnPointerUp(...)`에 `releasePointerCapture: true`를 전달합니다. WPF는 Bubble을 일반 mouse routed event로, Tunnel을 preview mouse event로 mapping하며 Avalonia는 같은 선택을 Bubble 및 Tunnel routing strategy로 mapping합니다. 이 neutral pointer callback 계약의 Duxel materialization은 parity 작업으로 남아 있습니다.
