...
The former Duxel-only `Nuri.DevTools` package has been removed. Applications reference only the matching `Nuri.WPF.Diagnostics` or `Nuri.Duxel.Diagnostics` package; shared inspector sources are compiled into each package and are not published as a third common package.
