...
Core pointer callbacks `.OnPointerDown(...)`, `.OnPointerMove(...)`, and `.OnPointerUp(...)` receive a `PointerEvent` whose `X` and `Y` are in the immediate parent layout coordinate space and whose `IsPrimaryButtonPressed` reports primary-button state. `EventRouting.Bubble` is the default; callers may select `EventRouting.Tunnel` when an ancestor or control class must not consume the interaction before the callback. Pointer capture is separately explicit: pass `capturePointer: true` to `.OnPointerDown(...)` and `releasePointerCapture: true` to `.OnPointerUp(...)` for drag interactions. WPF maps Bubble to normal mouse routed events and Tunnel to preview mouse events; Avalonia maps the same choices to its Bubble and Tunnel routing strategies. Duxel materialization of this neutral pointer callback contract remains parity work.
